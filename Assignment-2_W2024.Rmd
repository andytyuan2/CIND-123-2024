...
#INSERT YOUR ANSWER HERE
library(dplyr)
countsurvive <- titanicDataset %>% dplyr::count(age < 18, survived == 1, name = "survived_under") %>% sum(survived_under)
countsurvive
```
...
```{r}
#INSERT YOUR ANSWER HERE
titanicDataset %>% group_by(sex) %>% count(name = 'grouped_sex')
```
...
```{r}
#INSERT YOUR ANSWER HERE
titanicDataset %>% aggregate() %>% count()
```
...
titanicDataset %>% group_by(sex) %>% boxplot(fare, )
```
