...
If the sample size of **n** is given, then the z score can be written as:

$$
\frac{x-E[X]}{\sigma/\sqrt{n}}
$$
...
1. Uniform probability distributions
  ...
  - *If someone arrives at 8 AM, what is the probability of waiting more than 3 minutes but getting on the bus before 8:05 AM? The probability is distributed uniformly between 2 and 7.*
    - The probability of one minute is 1/(7-2), which is 1/5. So the probability of getting on a bus between 3 and 5 minutes is 0.4.
...
If a random sample of n measurements is selected from a large population with mean $\mu$ and standard deviation $\sigma$, the sampling distribution of the sample mean $\bar{x}$ will have mean $\mu$ and standard deviation $\frac{\sigma}{\sqrt{n}}$
...
2. Use the **rowMeans()** function to calculate the average of each possible pair in the previous step
```{r}
...
Simple LR: one x and one y

Multiple LR: multiple x and one y
...
It is possible to add prediction and confidence intervals to a scatter plot using **matlines()**, but there are a few caveats to take note of.

  - sometimes column values are in random order
  - prediction limits extend outside plot region
...
- Correlation does NOT measure the slope of the line, only if it is a line
$$
...
$$
s_{yy} = \sum (y_i - \bar{y})^2 = (computing formula)\sum_{i=1}^{n}y_i^2 - (\sum_{i=1}^{n}y_i^2)/n
$$


### Spearman correlation

- increase in y by increase by x
- monotonic relationship between the values: never increases or decreases
...
```{r}
# Test for correlation using the cor.test() function
cor.test(mtcars$mpg, mtcars$disp)
# which has a low p-value, making it significant, and a strong correlation of -0.84755
# Also, there is a 95% confidence interval between -0.92 and -0.71, which shows there would be a strong negative linear relationship either way
```

- in cor.test, t is the t-test statistic, df is the degrees of freedom, and the p-value is the significance of the test
  - t-test measures if there is a significant difference in the means of two groups
  - degrees of freedom is the number of independent variables
  - p-value is the significance of the test

```{r}
# To show the correlation between the pair-wise combinations of the variables in the data set, use the cor() function
cor(mtcars)
# Any variable's correlation with itself will be 1
```
